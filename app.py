...
# Encode image as Base64 (with safe fallback to a hosted placeholder)
BASE_DIR = Path(__file__).parent

def get_base64_image(image_path):
    """
    Attempt to load a local image (relative to app.py). If not found,
    return a placeholder image URL (1920x650) that the hero banner can use.
    The function returns either a data URI (when local file exists) or a URL string.
    """
    ...
    # Fallback: return placeholder URL with readable text
    text = quote_plus(Path(image_path).stem.replace("_", " ").title())
    return f"https://via.placeholder.com/1920x650.png?text={text}"

# Get Base64 image strings or placeholder URLs
...
<div style='background-color:#e6f2ff; padding:30px; border-radius:12px; text-align:center;'>
    ...
    <a href='/Risk_Assessment' target='_self'><button style='background-color:#4C9D70; color:white; padding:12px 24px; font-size:16px; border:none; border-radius:8px; cursor:pointer; transition:all 0.2s;'>Start Assessment</button></a>
</div>
<style>
